//
// How to deal with memcpy of non power of twos
//
// For resnet 50
//
/*
...
//
// Lets not worry about L2 cache on this one
//
...
__global__ void Kernel(float* output, float4_t* input, float4_t* filter) {
    ...
    if(tx == 0) {
        output[ty + bx * h + bz * h * w + by * k * h * w] = val;
    }
}
*/
...
//
// Next step, 
// store to a format batch norm can take, k, n, h, w
// double buffering
//
...
//
// ty + bx * h + bz * h * w + by * k * h * w
//
s_mul_i32 s9, s6, 56 // bx * h
v_add_u32 v4, v1, s9 // ty + bx * h
s_mul_i32 s9, s8, 56*56 // bz * h * w
v_add_u32 v4, v4, s9 // ty + bx * h + bz * h * w
s_mul_i32 s9, s7, 64 * 56 * 56 // by * k * h * w
v_add_u32 v4, v4, s9 // ty + bx * h + bz * h * w + by * k * h * w
